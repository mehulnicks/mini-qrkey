-- QSR Flutter App Database Schema for Supabase (Fixed Version)
...
-- Create RLS policies for restaurant_tables table
DROP POLICY IF EXISTS "Users can view all tables" ON restaurant_tables;
...
DROP POLICY IF EXISTS "Staff can view staff records" ON staff;
CREATE POLICY "Staff can view staff records" ON staff FOR SELECT USING (true);
...
DO $$
BEGIN
    RAISE NOTICE 'QSR Flutter App database schema has been successfully created!';
    RAISE NOTICE 'Tables created: orders, menu_items, categories, restaurant_tables, order_items, customers, staff';
    RAISE NOTICE 'Sample data inserted for categories, menu items, and tables';
