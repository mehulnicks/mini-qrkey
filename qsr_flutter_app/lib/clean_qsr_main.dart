...
import 'shared/widgets/enhanced_takeaway_dialog.dart';
import 'shared/models/core_models.dart';
import 'shared/services/scheduled_order_manager.dart';
import 'package:path_provider/path_provider.dart';

// Shared Components Imports
import 'shared/utils/date_formatter.dart';
import 'shared/utils/bill_formatter.dart';
import 'shared/widgets/common_widgets.dart';
import 'shared/widgets/order_card.dart';
import 'shared/widgets/menu_item_card.dart';
import 'shared/widgets/table_widgets.dart';
...
// Helper function for date time formatting - now using DateFormatter utility
...
// Models are now imported from shared/models/core_models.dart
...
class _OrderPlacementScreenState extends ConsumerState<OrderPlacementScreen> {
  ...
  void _placeOrder() {
    ...
    showDialog(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          print('DEBUG: Test Enhanced Takeaway button pressed');
          _showEnhancedTakeawayDialog();
        },
        backgroundColor: const Color(0xFFFF9933),
        foregroundColor: Colors.white,
        tooltip: 'Enhanced Takeaway Orders',
        child: const Icon(
  ...
  void _confirmPlaceOrder() {
    ...
    print('DEBUG: _confirmPlaceOrder called with orderType: $orderType');
    
    // Use enhanced dialog for takeaway orders
    if (orderType == OrderType.takeaway) {
      print('DEBUG: Launching enhanced takeaway dialog');
      _showEnhancedTakeawayDialog();
      return;
    }
    
    print('DEBUG: Using standard order flow for orderType: $orderType');
  ...
  void _showEnhancedTakeawayDialog() {
    print('DEBUG: _showEnhancedTakeawayDialog called');
    final currentOrder = ref.read(currentOrderProvider);
    final settings = ref.read(settingsProvider);

    print('DEBUG: Current order items: ${currentOrder.length}');
    ...
    if (currentOrder.isEmpty) {
      showDialog(
        ...
        builder: (context) => AlertDialog(
          ...
          content: const Text('Please add items to your cart before placing a takeaway order.'),
          actions: [
            TextButton(
              onPressed: () => Navigator.pop(context),
              child: const Text('OK'),
    ...
    // Show the full enhanced takeaway dialog directly
    _showFullEnhancedDialog();
  }

  void _showFullEnhancedDialog() {
    print('DEBUG: _showFullEnhancedDialog called');
    final currentOrder = ref.read(currentOrderProvider);
    final settings = ref.read(settingsProvider);
    
    // Create order charges for takeaway
    final orderCharges = _getOrderCharges(OrderType.takeaway, settings);

    // Get current order discount
    final orderDiscount = ref.read(currentOrderDiscountProvider);

    print('DEBUG: About to show EnhancedTakeawayOrderDialog');

    try {
      // Calculate total amount
      double subtotal = currentOrder.fold(0.0, (sum, item) => sum + (item.price * item.quantity));
      double totalWithCharges = subtotal + orderCharges.deliveryCharge + orderCharges.packagingCharge + orderCharges.serviceCharge;
      
      // Create schedule config
      final scheduleConfig = OrderScheduleConfig(
        allowScheduling: true,
        minAdvanceMinutes: 30,
        maxAdvanceDays: 7,
        workingHours: [
          WorkingHours(day: 'Monday', start: '09:00', end: '22:00'),
          WorkingHours(day: 'Tuesday', start: '09:00', end: '22:00'),
          WorkingHours(day: 'Wednesday', start: '09:00', end: '22:00'),
          WorkingHours(day: 'Thursday', start: '09:00', end: '22:00'),
          WorkingHours(day: 'Friday', start: '09:00', end: '22:00'),
          WorkingHours(day: 'Saturday', start: '10:00', end: '23:00'),
          WorkingHours(day: 'Sunday', start: '10:00', end: '21:00'),
        ],
      );
      
      // Create payment methods
      final paymentMethods = [
        PaymentMethodConfig(id: 'cash', name: 'Cash', isEnabled: true, processingFee: 0.0),
        PaymentMethodConfig(id: 'card', name: 'Card', isEnabled: true, processingFee: 0.02),
        PaymentMethodConfig(id: 'upi', name: 'UPI', isEnabled: true, processingFee: 0.0),
      ];
      
      showDialog(
        context: context,
        barrierDismissible: false,
        builder: (context) => EnhancedTakeawayOrderDialog(
        orderItems: currentOrder,
        totalAmount: totalWithCharges,
        scheduleConfig: scheduleConfig,
        paymentMethods: paymentMethods,
        onOrderPlaced: (Order completedOrder) {
          // Add order to the orders provider
          ref.read(ordersProvider.notifier).addOrder(completedOrder);
          print('DEBUG: Enhanced takeaway order added with status: ${completedOrder.status}');
          print('DEBUG: Order ID: ${completedOrder.id}');
          
          // Update customer analytics data if customer info is provided
          if (completedOrder.customer != null) {
          ...
          showOptimizedToast(
            context, 
            'Order #${completedOrder.id.substring(0, 8)} placed successfully!',
            icon: Icons.check_circle,
            duration: const Duration(seconds: 2),
          );
        },
      ...
      print('DEBUG: Error showing EnhancedTakeawayOrderDialog: $e');
      // Fallback to simple dialog
      showDialog(
        context: context,
        builder: (context) => AlertDialog(
          title: const Text('Enhanced Dialog Error'),
          content: Text('Error loading enhanced dialog: $e\n\nFalling back to standard order flow.'),
          actions: [
            TextButton(
              onPressed: () {
                Navigator.pop(context);
                // Continue with standard order flow by calling the rest of the original method
                _proceedWithStandardOrder();
              },
  ...
  void _proceedWithStandardOrder() {
    print('DEBUG: Proceeding with standard order flow');
    // Call the original order logic without the takeaway condition
    final currentOrder = ref.read(currentOrderProvider);
    final orderType = ref.read(orderTypeProvider);
    final settings = ref.read(settingsProvider);
    ...
    ref.read(currentOrderProvider.notifier).clearOrder();
    ref.read(currentOrderDiscountProvider.notifier).state = null;
    _customerNameController.clear();
    _customerPhoneController.clear();
    _customerEmailController.clear();
    _customerAddressController.clear();
